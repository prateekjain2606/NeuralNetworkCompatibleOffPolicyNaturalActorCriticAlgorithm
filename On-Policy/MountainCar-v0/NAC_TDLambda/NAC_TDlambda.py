...
def trainIters(actor, v_critic, a_critic, n_iters):
    ...
    for iter in range(n_iters):
        ...
        for j in range(len(v_critic.all_layers_sizes)-1):
            z_weights.append(torch.zeros(v_critic.all_layers_sizes[j+1], v_critic.all_layers_sizes[j]).to(device))
        for j in range(1,len(v_critic.all_layers_sizes)):
            z_bias.append(torch.zeros(v_critic.all_layers_sizes[j]).to(device))
        ...
        for i in count():
            ...
            if done:
                ...
                v_value.backward()
                for j in range(len(v_critic.all_layers_sizes)-1):
                    z_weights[j] = gamma*lamda*z_weights[j] + v_critic.linears[j].weight.grad
                    z_bias[j] = gamma*lamda*z_bias[j] + v_critic.linears[j].bias.grad
                for j in range(len(v_critic.all_layers_sizes)-1):
                    v_critic.linears[j].weight.grad = -1*2*delta*z_weights[j]
                    v_critic.linears[j].bias.grad = -1*2*delta*z_bias[j]
                optimizerV_C.step()
                optimizerV_C.zero_grad()
                ...
                v_value = v_critic(state)
                next_v_value = v_critic(next_state)
                delta = (reward + gamma * next_v_value).detach()
                error = delta - a_value
                a_critic_loss = error.pow(2)
                optimizerA_C.zero_grad()
                a_critic_loss.backward()
